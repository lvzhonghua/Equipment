	...
	public partial class FormMain : Form
	{
		...
		InitCOM:		//自适应连接COM口			
			...
			catch (IOException)
			{
				if (portID > 20)
				{
					MessageBox.Show("USB Can not open", "Linker Error",
            ...
            try
            {
                ...
                switch (cmdTemp.Code)
                {
                    ...
                    case CommandCode.START:	//START 开始发气量采集                   
                        dataValue = "Start Gas Collecting";
                        this.isVG_0 = true;
                    ...
                    case CommandCode.END:	 //END 结束发气量采集
                        dataValue = "Gas Colecting Finished";
                    ...
                    case CommandCode.E0:	//E0：热电偶通道开路错误	E1：压力传感器通道开路错误	E2：系统温度传感器通道开路错误
                        //dataValue = "热电偶通道开路错误";
                        dataValue = "Thermocouple Channel Error";
                        this.lblCurrentFurnaceTemprature.Text = "Err0";
                        //MessageBox.Show("热电偶通道开路错误", "请检查硬件错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        MessageBox.Show(dataValue, "Please check hardware", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        break;
                    case CommandCode.E1:
                        //dataValue = "压力传感器通道开路错误";
                        dataValue = "Pressure Sensor Channel Error";
                    ...
                    case CommandCode.E2:
                        ...
                        dataValue = "System Temperature Sensor Channel Error";
                    ...
                    case CommandCode.O0:	//OK0：热电偶通道正常		OK1：压力传感器通道正常	OK2：系统温度传感器通道正常	
                        //dataValue = "热电偶通道正常";
                        dataValue = "Thermocouple Channel is OK";

                        break;
                    case CommandCode.O1:
                        //dataValue = "压力传感器通道正常";
                        dataValue = "Pressure Sensor Channel is OK";

                        break;
                    case CommandCode.O2:
                        //dataValue = "系统温度传感器通道正常";
                        dataValue = "System Temperature Sensor Channel is OK";

                        break;
                    case CommandCode._0:	//仪器在线查询指令返回结果
                        //dataValue = "仪器在线";
                        dataValue = "Instrument is online";
                        break;
                    case CommandCode._1:	//发送加热炉控制指令返回结果
                        //dataValue = "加热炉温控制指令正常执行";
                        dataValue = "Heating furnace temperature control command is executed normally";
                        break;
                    case CommandCode._2:	//发送系统控制指令返回结果
                        //dataValue = "系统控制指令正常执行";
                        dataValue = "The system control command is executed normally";
                        break;
                    case CommandCode._3:	//请求重新发送上一个压力采样数据
                        //dataValue = "重新发送上一个压力采样数据";
                        dataValue = "Resend last pressure sample data";
                        break;
                    case CommandCode._4:	//开始实验指令返回结果
                        //dataValue = "开始实验指令正常执行";
                        dataValue = "Start the experiment and execute the instruction normally";
                        break;
                    case CommandCode._5:	//结束实验指令返回结果
                        //dataValue = "结束实验指令正常执行";
                        dataValue = "The end of the experiment instruction is executed normally";

                        break;
                    case CommandCode._6:	//开始发送数据指令返回结果
                        //dataValue = "开始发送数据指令正常执行";
                        dataValue = "Start sending data, and the command is executed normally";
                        break;
                    case CommandCode._E:	//指令出错
                        //dataValue = "指令执行出错";
                        dataValue = "Instruction execution error";
                        break;
...
//////////////////////////////按键相关//////////////////////////////////////////
		...
		private void btnStart_Click(object sender, EventArgs e)
		{
			...
			catch (InvalidOperationException)
			{
                ...
                MessageBox.Show("Operation error. The connection between master and slave may be disconnected.\r\nPlease check the USB port cable.",
                                           "Error Information", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
			catch
			{
			}
        ...
        private void EndGasTest()
        {
            try
            {
                ...
                groupBoxGas.Text = "Max Gas";
                this.lblGas.Text = this.maxGas.ToString();
                this.groupBox2.Text = "Max Gas Time: mm:ss";
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            saveDlg.Filter = "Sample parameters and gas(*.gas)|*.gas";
        ...
        {
            SaveFileDialog saveDlg = new SaveFileDialog();
            saveDlg.Filter = "Sample parameters and gas(*.txt)|*.txt";
            ...
            this.panelG.Visible = false;
            this.timerDelay.Enabled = true;
            DialogResult dalResult = MessageBox.Show("Do you want to save the print data？", "Confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
        ...
        {
            if (this.btnSetting.Text == "Modify sample")
            {
            ...
            else if (this.btnSetting.Text == "OK")
            {
                if (this.txtFactory.Text.Length > 12)
                {
                    MessageBox.Show("Factory should be less than 12 characters", "Tips");
                    return;
                }
                if (this.txtSampleNumber.Text.Length > 12)
                {
                    MessageBox.Show("The sample No. should be less than 12 characters", "Tips");
                    return;
                }
                if(this.txtSampleName.Text.Length>12)
                {
                    MessageBox.Show("The sample Name should be less than 12 characters", "Tips");
                    return;
                }
        ...
        private void btnLoad_Click(object sender, EventArgs e)
        {
            ...
            catch (Exception ex)
            {
                ...
                MessageBox.Show("Error starting GasDataReader program：" + ex.Message, "Error Information",
                                            MessageBoxButtons.OK,
                                            MessageBoxIcon.Error);
            }
        ...
        {
            DialogResult dalResult = MessageBox.Show("Are you sure you want to exit the program?", "Confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
        ...
        private void btnCustom_OK_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (targetTemprature > 1050)
                {
                    ...
                    MessageBox.Show("The target temperature should not exceed 1050 ℃", "Tips", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            ...
            catch (FormatException)
            {
                ...
                MessageBox.Show("Please enter the correct number", "Tips", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                this.txtCustom.Focus();
            }
        }
